...
class ColumnSelector:
    def __init__(self, data: pd.DataFrame, num_classes: int = 5):
        ...
        self.logger.info(f"Total de colunas: {len(data.columns)}")
    ...
    def get_numerical_columns(self) -> List[str]:
        ...
        if numerical_columns:
            ...
            self.logger.info(f"Exemplos: {numerical_columns[:5]}...")
    ...
    def get_nominal_columns(self) -> List[str]:
        ...
        if nominal_columns:
            ...
            self.logger.info(f"Exemplos: {nominal_columns[:5]}...")
    ...
    def get_ordinal_columns(self) -> List[str]:
        ...
        if ordinal_columns:
            ...
            self.logger.info(f"Exemplos: {ordinal_columns[:5]}...")
    ...
    def get_ordinal_categories(self) -> Dict[str, List]:
        ...
        if not ordinal_columns:
            return None
        ordinal_categories = {
            col: self.data[col].unique().tolist() for col in ordinal_columns}
        return ordinal_categories if ordinal_categories else None

    def get_columns_by_regex(self, regex_pattern: str) -> List[str]:
        columns_by_regex = self.data.filter(
            regex=regex_pattern).columns.tolist()
