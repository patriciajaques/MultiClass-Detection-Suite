...
class Evaluation:
    ...
    def evaluate_all_models(trained_models, X_train, y_train, X_test, y_test):
        ...
        for model_name, model_info in trained_models.items():
            ...
            try:
                ...
                print(f"CV score no evaluate_all_models: {cv_score}")
                ...
                if 'feature_selection' in pipeline.named_steps:
                    ...
                    print(
                        f"Shape após feature selection: {X_train_transformed.shape}")
                ...
                train_metrics = Evaluation._generate_metrics(
                    y_train, y_train_pred, y_train_prob)
                test_metrics = Evaluation._generate_metrics(
        ...
        try:
            ...
            if hasattr(selector, 'components_'):
                ...
                if hasattr(selector, 'explained_variance_ratio_'):
                    ...
                    feature_info['cumulative_variance'] = np.cumsum(
            ...
            # Para seletores baseados em máscara (RF, RFE, MI)
                ...
                if hasattr(X_train, 'columns'):
                    ...
                    selected = [f'feature_{i}' for i in range(
                        len(mask)) if mask[i]]
    ...
    def _generate_metrics(y_true, y_pred, y_prob):
        ...
        # Create DataFrame without accuracy entry and other aggregate metrics
        class_report = pd.DataFrame(report_dict).transpose()
        class_report = class_report.drop(
        ...
        for metric in ['balanced_accuracy', 'accuracy', 'kappa']:
            # usa accuracy para 'accuracy'
            value = metrics_dict.get(metric, accuracy)
            metrics_df.loc[metric] = [
                value,     # precision column
                value,     # recall column
                value,     # f1-score column
